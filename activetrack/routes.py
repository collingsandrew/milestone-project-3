    ...
    if request.method == 'POST':
        ...
        if user_name:
            ...
            try:
                ...
                flash(f'Error, failed to sign up: {str(e)}', category='error')
...
def diary():
    try:
        ...
        flash(f'Error, failed to load activities: {str(e)}', category='error')
        return redirect(url_for('home'))
...
def add_activity():
    if request.method == "POST":
        try:
            ...
            db.session.rollback()
            # catch the error and flash a message
            flash(f'Error, failed to add an activity: {str(e)}', category='error')
...
def edit_activity(activity_id):
    ...
    if request.method == "POST":
        try:
            ...
            db.session.rollback()
            # catch the error and flash a message
            flash(f'Error, failed to edit activity: {str(e)}', category='error')
...
def delete_activity(activity_id):
    ...
    try:
        ...
        db.session.rollback()
        # catch the error and flash a message
        flash(f'Error, failed to delete activity: {str(e)}', category='error')
...
def get_exercises():
    try:
        ...
        flash(f'Error, failed to load JSON file: {str(e)}', category='error')
        return redirect(url_for('diary'))
    except Exception as e:
        # if there is a general error
        flash(f'Error, failed to load exercises: {str(e)}', category='error')
        return redirect(url_for('diary'))
...
def activity_feed():
    try:
        ...
        # catch the error and flash a message, redirect to home page
        flash(f'Error, failed to load activity feed: {str(e)}', category='error')
        return redirect(url_for('home'))
...
def add_comment():
    if request.method == "POST":
        try:
            ...
            db.session.rollback()
            # catch the error and flash a message
            flash(f'Error, failed to add comment: {str(e)}', category='error')
...
def delete_comment(comment_id):
    ...
    try:
        ...
        db.session.rollback()
        # catch the error and flash a message
        flash(f'Error, failed to delete comment: {str(e)}', category='error')
    return redirect(request.referrer or url_for('activity_feed'))
